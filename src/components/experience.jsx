                ...
                <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
                    ...
                    <motion.div whileInView={{ opacity: 1, x: 0 }} initial={{ opacity: 0, x: 100 }} transition={{ duration: 0.5}} className="w-full max-w-xl lg:w-3/4">
                        <h6 className="mb-2 font-semibold">
                            {experience.role} - <span className="text-sm text-cyan-200">{experience.company}</span>
                        </h6>
                        <p className="mb-4 text-neutral-400">{experience.description}</p>
                        {experience.technologies && (
                            <div className="flex flex-wrap">
                                {experience.technologies.map((technology, index) => (
                                    <div key={index} className="mr-2 mt-4 rounded bg-neutral-900 px-2 py-1 text-sm font-medium text-cyan-700">
                                        {technology}
                                    </div>
                                ))}
                            </div>
